...
class FlowNetwork{
    ...
    int DFS(int cur, int f){
        ...
        for(int &i = savedPointDFS[cur]; i < graph[cur].size(); ++i){
            ...
            if(pushedFlow){
                ...
                if(edge.u<n && edge.flow>0){workers.insert(edge.u);}
                else if(edge.u<n){workers.erase(edge.u);}
    ...
    public:
    FlowNetwork(int n, int v):n(n),V(v),graph(V),visited(V),dist(V){}
...
int main(){
    ...
    cin.tie(0);
    int n,m; cin>>n>>m;
    FlowNetwork fn(n,n+m+2);
    for(int i = 0; i < n; ++i){
        int k; cin>>k;
        fn.addEdge(n+m,i,1,0);
        for(int j = 0; j < k; ++j){
            int task, wage; cin>>task>>wage; --task;
            fn.addEdge(i,n+task,1,(10000-wage));
        }
    }
    for(int i = 0; i < m; ++i){
